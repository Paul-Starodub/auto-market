...
async def login(
    ...
    customer = await customers_crud.get_customer_by_email(db=db, email=form_data.username)
    ...
    await customers_crud.delete_expired_refresh_tokens(db, customer.id)
    jwt_refresh_token = jwt_manager.create_refresh_token({"sub": str(customer.id)})
    try:
        await customers_crud.create_refresh_token(
            db=db,
            customer_id=customer.id,
            token=jwt_refresh_token,
            days_valid=settings.refresh_token_expire_days,
        )
        await customers_crud.cleanup_old_refresh_tokens(db=db, customer_id=customer.id, keep_latest=5)
...
async def refresh_tokens(
    ...
    db_token = await customers_crud.get_refresh_token(db=db, token=refresh_token)
    if not db_token:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Refresh token not found")
    if db_token.expires_at < datetime.now(UTC):
        await customers_crud.delete_refresh_token(db=db, token=refresh_token)
...
async def reset_password(request_data: ResetPasswordRequestSchema, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    if not customer:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid or expired reset token")
    customer.password_hash = hash_password(request_data.new_password)
    await customers_crud.delete_password_reset_token_by_user(db=db, customer_id=customer.id)
    await db.commit()
...
async def change_password(
    ...
    await customers_crud.delete_password_reset_token_by_user(db, current_customer.id)
    await db.commit()
...
async def update_customer(
    ...
    if customer_update.username and customer_update.username.lower() != customer.username.lower():
        existing = await customers_crud.get_customer_by_username(db=db, username=customer_update.username)
        if existing:
            raise HTTPException(
    ...
    if customer_update.email and customer_update.email.lower() != customer.email:
        existing = await customers_crud.get_customer_by_email(db=db, email=customer_update.email)
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Email already registered",
            )
    return await customers_crud.update_customer(db=db, customer=customer, customer_update=customer_update)
...
async def delete_customer(
    ...
    if customer_id != current_customer.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to delete this customer")
    customer = await customers_crud.get_customer_by_id(db=db, customer_id=customer_id)
    if not customer:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Customer not found")
...
async def upload_customer_picture(
    ...
    if current_customer.id != customer_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to update this customer's picture"
        )
...
async def delete_customer_picture(
    ...
    if current_customer.id != customer_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to delete this customer's picture"
        )
    old_filename = current_customer.image_file
...
async def create_my_profile(
    ...
    existing = await customers_crud.get_profile_by_customer_id(db=db, customer_id=current_customer.id)
    if existing:
...
async def update_my_profile(
    ...
    if not profile:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Profile not found")
    return await customers_crud.update_profile(db=db, profile=profile, data=data)
...
async def delete_my_profile(current_customer: CurrentCustomer, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    if not profile:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Profile not found")
    await customers_crud.delete_profile(db=db, profile=profile)
