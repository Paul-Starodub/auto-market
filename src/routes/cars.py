...
async def upload_car_images(
    ...
    images = await cars_crud.add_car_images(db=db, car_id=car_id, file_paths=saved_file_paths)
    return images
...
async def delete_car(car_id: int, db: Annotated[AsyncSession, Depends(get_db)]):
    car = await cars_crud.delete_car(db=db, car_id=car_id)
    if not car:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Car not found")
